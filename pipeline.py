...
import time
import traceback
from DePT_src.Updater_DePT import Updater_DePT
...
class Pipeline:
    ...
    def run(self, multi_process=False):
        ...
        if self.dic_exp_conf['pargs']['onlyEva']:
            self.dic_traffic_env_conf['onlyEva'] = True
            
            test_evaluation_start_time = time.time()
            if multi_process:
                p = Process(target=model_test.test,
                            args=(self.dic_path["PATH_TO_DATA"], self.dic_exp_conf['pargs']['onlyEva_modelPath'], self.dic_exp_conf['pargs']['onlyEva_cntRound'], self.dic_exp_conf["RUN_COUNTS"], self.dic_traffic_env_conf, False))
                p.start()
        ...
        for cnt_round in range(self.dic_exp_conf["NUM_ROUNDS"]):
            ...
            if multi_process:
                p = Process(target=model_test.test,
                            args=(self.dic_path["PATH_TO_DATA"], self.dic_path["PATH_TO_MODEL"], cnt_round, self.dic_exp_conf["RUN_COUNTS"], self.dic_traffic_env_conf, False))
                p.start()
                if self.dic_exp_conf["EARLY_STOP"]:
                    p.join()
            else:
