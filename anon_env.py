...
from config import Process, Pool
...
class Intersection:
    ...
    def _update_feature_map(self, simulator_state):
        ...
        dic_feature["cur_time"] = self._get_cur_time(simulator_state)
    ...
    def _get_cur_time(self, simulator_state):
        return simulator_state['get_cur_time']
...
class AnonEnv:
    ...
    def reset(self):
        ...
        if self.dic_traffic_env_conf["FAST_COMPUTE"]:
            self.system_states = {"get_lane_vehicles": self.eng.get_lane_vehicles(),
                              ...
                              "get_cur_time": self.eng.get_current_time()
                              }
        else:
            self.system_states = {"get_lane_vehicles": self.eng.get_lane_vehicles(),
                              "get_lane_waiting_vehicle_count": self.eng.get_lane_waiting_vehicle_count(),
    ...
    def step(self, action):
        ...
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]):  # 10
            ...
            self._inner_step(action_in_sec)  # wenqing
    ...
    def batch_log(self, start, stop):
        for inter_ind in range(start, stop):
            ...
            df.to_csv(path_to_log_file, na_rep="nan")  # 1

            path_to_log_file2 = os.path.join(self.path_to_log, "inter_{0}.pkl".format(inter_ind))
            f = open(path_to_log_file2, "wb")
            pickle.dump(self.list_inter_log[inter_ind], f)  # 2
            f.close()
